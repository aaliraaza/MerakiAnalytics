...
DROP TABLE IF EXISTS proj_galedb.gros_Tenure;
CREATE TABLE IF NOT EXISTS proj_galedb.gros_Tenure
(
...
STORED AS ORC
LOCATION 'wasbs://lab@rgdsadldev.blob.core.windows.net/PROJ_GaleMicroClustering/features/gros_Tenure';

INSERT OVERWRITE TABLE proj_galedb.gros_Tenure
...
SELECT t1.customergoldbrandid,
        ...
        case when floor(datediff(current_timestamp,min(t2.registrationdate))/365) is null then '< 1 Year'
             ...
             when floor(datediff(current_timestamp,min(t2.registrationdate))/365) > 6  then '7+ Years'
