...
DROP TABLE IF EXISTS proj_galedb.mecc_PercVisits;
CREATE TABLE IF NOT EXISTS proj_galedb.mecc_PercVisits
(
...
STORED AS ORC
LOCATION 'wasbs://lab@rgdsadldev.blob.core.windows.net/PROJ_GaleMicroClustering/features/mecc_PercVisits';

INSERT OVERWRITE TABLE proj_galedb.mecc_PercVisits

SELECT customergoldbrandid,sum(percMorn) as percMorn, sum(percAfternoon) as percAfternoon, sum(percEvening) as percEvening
FROM (
